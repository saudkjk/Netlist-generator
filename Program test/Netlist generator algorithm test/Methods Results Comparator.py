...
def compare_netlists(ground_truth, test_netlist):
    ...
    used_components = set()  # Tracks test components already used in matching

    def is_equivalent(ground_component, test_component, locked_nodes, node_mapping):
        ground_nodes = ground_component.nodes
        test_nodes = apply_mapping(test_component.nodes, node_mapping)

        for i in range(len(ground_nodes)):
            ground_node = ground_nodes[i]
            test_node = test_nodes[i]

            # Check locked nodes
            if ground_node in locked_nodes and locked_nodes[ground_node] != test_node:
                return False

            # Check node mapping
            if ground_node in node_mapping:
                if node_mapping[ground_node] != test_node:
                    return False

            # Check for conflicting test node reuse
            if test_node in node_mapping.values() and not any(
                ground_node == k and test_node == v for k, v in node_mapping.items()
            ):
                return False
    ...
    def apply_mapping(nodes, node_mapping):
        # Replace test nodes with their mapped equivalents (or keep as-is if unmapped)
        return [node_mapping.get(node, node) for node in nodes]

    def backtrack(ground_index, current_mapping, locked_nodes):
        if ground_index >= len(ground_truth):
            return True  # All components matched successfully

        ground_component = ground_truth[ground_index]

        for test_component in test_netlist:
            if test_component in used_components:
                continue  # Skip already used test components

            # Check if test_component set matches ground_component (order-independent)
            if is_set_match(ground_component, test_component):
                used_components.add(test_component)
                original_mapping = current_mapping.copy()

                for i in range(len(ground_component.nodes)):
                    ground_node = ground_component.nodes[i]
                    current_mapping[ground_node] = ground_node

                if backtrack(ground_index + 1, current_mapping, locked_nodes):
                    return True
        ...
        for test_component in test_netlist:
            ...
            if len(test_component.nodes) != len(ground_component.nodes):
            ...
            for i in range(len(ground_component.nodes)):
                ...
                if ground_node not in current_mapping:
                    current_mapping[ground_node] = test_node
